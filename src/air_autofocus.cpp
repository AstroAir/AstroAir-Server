...
namespace AstroAir
{
    int StarIndex,AImageWeight,AImageHeight;
    double HFD;
    AUTOFOCUS *autofocus;
    ...
    bool AUTOFOCUS::SolveImageInfo(std::string FileName)
    {
        ...
        // 计算星点数 --> In: 原始图像（降噪部分由于效率问题已删除）, Out: 星点数量
        ...
        for (StarInfoListT::iterator it = starInfos.begin(); it != starInfos.end(); ++it) {
            ...
            */
            ...
            oss.precision(4);
            oss    << "HFD=" << hfd << std::endl
            << "FWHM H=" << fwhmHorz << std::endl
            << "FWHM V=" << fwhmVert << std::endl
            << "MAX=" << (int)maxPixValue << std::endl
            << "SAT=" << (saturated ? "Y" : "N");
        ...
        HFD = hfd_temp / starInfos.size();
        rgbResized.save("autofocus_temp.jpg");      //保存处理之后的图像
        return true;
    }

    bool AUTOFOCUS::insideCircle(float inX /*pos of x*/, float inY /*pos of y*/, float inCenterX, float inCenterY, float inRadius)
    {
    return (pow(inX - inCenterX, 2.0) + pow(inY - inCenterY, 2.0) <= pow(inRadius, 2.0));
    }

    void AUTOFOCUS::readFile(CImg<float> &inImg, const string &inFilename, long *outBitPix)
    {
    ...
    void AUTOFOCUS::thresholdOtsu(const CImg<float> &inImg, long inBitPix, CImg<float> *outBinImg)
    {
    ...
    void AUTOFOCUS::getAndRemoveNeighbours(PixelPosT inCurPixelPos, PixelPosSetT *inoutWhitePixels, PixelPosListT *inoutPixelsToBeProcessed)
    {
    ...
    template <typename T>
    void AUTOFOCUS::clusterStars(const CImg<T> &inImg, StarInfoListT *outStarInfos)
    {
    ...
    float AUTOFOCUS::calcIx2(const CImg<float> &img, int x)
    {
        float Ix = 0;
    ...
    float AUTOFOCUS::calcJy2(const CImg<float> &img, int y)
    {
        float Iy = 0;
    ...
    void AUTOFOCUS::calcIntensityWeightedCenter(const CImg<float> &inImg, float *outX, float *outY)
    {
        assert(outX && outY);
    ...
    void AUTOFOCUS::calcSubPixelCenter(const CImg<float> &inImg, float *outX, float *outY, size_t inNumIter)
    {
    ...
    void AUTOFOCUS::calcCentroid(const CImg<float> &inImg, const FrameT &inFrame, PixSubPosT *outPixelPos, PixSubPosT *outSubPixelPos, size_t inNumIterations)
    {
        // Get frame sub img
    ...
    */
    float AUTOFOCUS::calcHfd(const CImg<float> &inImage, unsigned int inOuterDiameter)
    {
